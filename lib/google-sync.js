...
function createGoogleSync({ db, rootDir, log = console }) {
  ...
  async function handleCallback({ code, state, empCode = null }) {
    ...
    // Google's consent screen has a checkbox per permission; if the user left the
    // calendar one unticked we get a token without it and every API call would 403.
    const granted = String(tok.scope || '').split(/\s+/);
    if (!granted.includes(CALENDAR_SCOPE)) {
      try { await fetch(`${REVOKE_URL}?token=${encodeURIComponent(tok.refresh_token)}`, { method: 'POST' }); } catch {}
      throw new Error('ยังไม่ได้ให้สิทธิ์ปฏิทิน — ตอนหน้า Google ถามสิทธิ์ ให้ติ๊กช่อง "สร้างปฏิทิน Google รอง…" ให้ครบ แล้วกด "ดำเนินการต่อ" จากนั้นลองเชื่อมต่อใหม่');
    }
